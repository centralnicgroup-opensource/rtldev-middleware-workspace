...
#
# Denies three specific shapes only — each one dumps an unbounded amount of file
# content into context when a cheaper tool exists:
#
...
# pipeline (`pnpm lint | grep -c error`), `cat` feeding a pipe, and heredocs are
# all legitimate and stay allowed — they are already bounded.
#
# Language-agnostic: nothing here is specific to a project, so it can be adopted
# as-is. Fails open — any unexpected input exits 0 without a decision.
